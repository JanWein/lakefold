...
Review of lakefold 0.6.0, 19 September 2026.
...
## Review findings addressed in 0.6.0

| Finding | Implemented correction |
|---|---|
| Abbreviated formulas could share an identity | Full expression fingerprints; legacy metric versions cannot be silently reinterpreted |
| Data pronouns could bypass missing-value checks | Static column extraction, explicit inputs for unresolved expressions and batched null checks |
| Correcting August hid a retained September delivery | Check the current partitioned release's actual date values |
| Framework IDs changed the global random stream | RNG-independent identifiers with uniqueness regression coverage |
| Simple month imports required a pipeline | Optional `partition_by` on `dl_write()` and `dl_ingest()` |
| Automatic integer inference blocked later decimals | Compatible automatic numeric schemas; explicit integer contracts stay strict |
| Real source column names were rejected | Quoted column names across contracts, metrics, partitions and comparisons |
| Analyses always wrote metadata | Database-enforced read-only connections and optional measurement recording |
| Recalculating a report broke an identical retry | Compare semantic content, retain original timestamps and expose report readback |
| Original quality exceptions were discarded | Optional local conditions without persisting their raw text |
| Interrupted staging could block the next import | Explicit recovery with live-process protection, preview and retained evidence |
| Change analysis needed ad hoc code | Keyed release comparisons, bounded row previews and numeric summaries |
| APIs needed separate ingestion boilerplate | A zero-argument source function can feed `dl_write()` directly |

These changes improve ordinary usage and correctness. They do not turn a
single-table publication into an atomic multi-table transaction. Advanced
capabilities below need their own design and evidence.
...
| Medium | Stream large external sources | Extend the existing snapshot callback with database-side fingerprints and streaming semantics |
...
Metric calculation can run without writes using a read-only connection or
`record = FALSE`. Ordinary recorded calculations deduplicate identical lineage
edges. Read-only access does not add distributed writer coordination.
...
| 0.6.0 | Integrity corrections, simple partition imports, release comparison, read-only analysis, report readback, source callbacks and explicit recovery |
