...
```r
result <- tw_run(orders_definition, evidence = "runs", stop_on_failure = FALSE)
tw_run_history("runs")
tw_incidents("runs")
tw_read_run("runs", result$run_id)
```
...
`tw_quality_errors()` or retained pointblank agents. Raw conditions may include values
...
```r
lineage <- tw_catalog_openlineage(
...
metadata <- tw_catalog_openmetadata(
...
orders_definition <- tw_product("orders") |>
  tw_add_source("orders.csv", name = "delivery") |>
  tw_add_catalog(lineage) |>
  tw_add_catalog(metadata)
tw_run(orders_definition, evidence = "runs")
```
...
database and schema must already exist. `source_tables` maps `tw_add_source()` names
...
```r
tw_retry_catalogs("runs", list(lineage, metadata))
tw_run_history("runs")
```

Only pending deliveries with matching destination IDs are attempted. A callback
registered with `tw_add_catalog(callback, name = "audit")` must be supplied as
...
`tw_transform_dbt()` does this for its factory-owned connections. Other callers must
...
`tw_target_database(mode = "append")` checks the complete existing and incoming
...
```r
tw_init_project("orders-project", targets = TRUE, renv = TRUE, connect = TRUE)
```
...
`tw_interrupted(lake)` lists runs left in `running` state. Stop the original writer,
then preview recovery for explicit run IDs:

```r
tw_recover(lake, run_ids = selected_run_id, staging_assets = "orders")
...
`tw_cleanup(lake)` previews eligible unpublished tables from completed failed runs;
...
`tw_freshness()` and `tw_check_delivery()` provide checks for an external scheduler;
there is no background monitoring service. `tw_report_read()` reads saved report
