...
`dbt_project(path, lake = config, sources = published_products)`.
`run()` prepares the managed connection profile and exact release bindings.
...
The walkthrough exports its R structural contracts with `dbt_contract()` into
...
writes. `built |> publish("monthly_performance", contract = mart_contract)`
