...
This ordinary dbt-duckdb project accompanies the synthetic tidyweave tutorial.
Copy it to a working directory, then define
`tw_dbt_project(path, lake = config, sources = published_products)`.
`tw_run()` prepares the managed connection profile and exact release bindings.
...
The walkthrough exports its R structural contracts with `tw_dbt_contract()` into
...
writes. `built |> tw_publish("monthly_performance", contract = mart_contract)`
