---
title: "Why tidyweave?"
description: "Decide whether repeated deliveries and report history justify the package."
...
Read this to decide whether tidyweave fits your problem. For the first
hands-on example, go directly to [Get started](get-started.html).
...
orders <- tw_product("orders", data.frame(id = 1:2, amount = c(100, 250))) |>
  tw_add_quality(~ amount >= 0)

result <- tw_trial(orders)
sum(tw_collect(result)$amount)
stopifnot(sum(tw_collect(result)$amount) == 350)
```
...
| A check fails | Inspect `tw_quality_report(result)` | A named failed check and counts |
...
Continue with the [five introductory lessons](get-started.html). The
