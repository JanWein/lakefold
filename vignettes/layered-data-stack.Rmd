...
checked <- tw_product("orders", orders) |>
...
Configure the four layers explicitly. `tw_lake_config()` describes a lake without
...
ducklake_config <- tw_lake_config(
  tw_registry_duckdb("ducklake-stack/metadata.ducklake"),
  tw_storage_local("ducklake-stack/data"),
...
accepted <- tw_ingest(orders, quality = ~ amount >= 0, to = config)
tw_collect(accepted)
accepted$outputs
```

`tw_ingest()` infers the asset name from `orders`. With a file, it can infer the name
...
accepted <- tw_ingest("deliveries/orders.csv",
...
Pass this as `contract = order_contract` to `tw_ingest()`. It adds the declared schema
...
accepted <- tw_product("orders", orders) |>
  tw_add_contract(order_contract) |>
...
`tw_pointblank_checks()` to `tw_add_quality()`. The optional adapter extends the simple
...
rejected <- tw_ingest(bad_orders, name = "orders",
...
new_raw <- tw_ingest(next_orders, name = "orders",
...
An optional `tw_dbt_contract()` converts supported schema expectations into normal
dbt model properties:

```{r export-contract}
properties <- tw_dbt_contract(order_contract, "stg_orders")
...
revenue_contract <- tw_contract(
  columns = c(customer_id = "integer", revenue = "numeric"),
  key = "customer_id",
  rules = list(tw_quality_rule("non_negative_revenue", ~ revenue >= 0))
)
approved <- tw_publish(built, "customer_revenue", to = config,
...
returned by `tw_collect(approved)`.
...
saveRDS(tw_collect(approved), "approved-revenue.rds")
...
exported <- tw_product("revenue_export", approved) |>
  tw_set_target(tw_target_parquet("approved-revenue.parquet")) |>
  tw_run()
```
...
Use `tw_target_database(connection, "customer_revenue")` instead to write an approved
...
open. These ownership differences remain visible through adapter inspection. A simpler `tw_add_source(tw_collect(approved))` also works when only
...
If your team operates OpenMetadata, `tw_catalog_openmetadata_dbt()` can hand these
artifacts to OpenMetadata's **official Python ingestion engine**. This is separate
from `tw_catalog_openmetadata()`, which publishes ordinary product table metadata
over HTTP.
...
built <- project |> tw_run(catalog = catalog, echo = FALSE)
built$success
...
`tw_publish_metadata(catalog, built)`. The automatic hook is appropriate when the
...
delivery <- tw_publish_metadata(catalog, built)
delivery$status
```
...
delivery <- tw_publish_metadata(catalog, built, force = TRUE)
```
...
result, or an extension can implement `tw_publish_metadata()` and declare
