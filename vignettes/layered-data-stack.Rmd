---
title: "Build a layered data stack, one step at a time"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Build a layered data stack, one step at a time}
...
accepted <- ingest(config, orders, quality = ~ amount >= 0)
collect(accepted)
accepted$outputs
```
...
accepted <- ingest(config, "deliveries/orders.csv",
  name = "orders", quality = ~ amount >= 0)
```
...
Pointblank is another optional way to express the input gate. Replace the native
predicate with a pointblank adapter when its richer checks are useful:

```{r pointblank-input, eval=requireNamespace("pointblank", quietly=TRUE)}
...
```{r use-pointblank-input, eval=FALSE}
accepted <- ingest(config, orders, contract = order_contract, quality = checks)
```

Both engines run at the same input boundary. Additional business checks are
optional; a malformed result or a blocking failure prevents acceptance.
...
rejected <- ingest(config, bad_orders, name = "orders",
  quality = ~ amount >= 0, stop_on_failure = FALSE)
...
new_raw <- ingest(config, next_orders, name = "orders",
  quality = ~ amount >= 0)
dbt_sources(project, list(orders = new_raw))
```

Only the generated source binding changes; the SQL can keep using
...
exported <- product("revenue_export") |>
  add_source(source_release(config, approved$asset, approved$release_id)) |>
...
`source_release()` preserves pinned release lineage in the export's execution
metadata. With a configuration, it opens and closes its own read-only connection
and returns an in-memory table. Use a caller-owned connected lake when the source
should stay lazy. A simpler `add_source(collect(approved))` also works when only
...
built <- dbt_build(project, catalog = catalog, echo = FALSE)
built$success
