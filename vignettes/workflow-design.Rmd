...
corrected_orders <- orders |>
  replace_sources(orders = data.frame(id = 1:2, amount = c(10, 25)))
corrected_orders |> run() |> collect()
```

A primary source supplied to `product("orders", data)` defaults to the name
...
`measurements |> report_release(id, code_version = ...)` issues the original
