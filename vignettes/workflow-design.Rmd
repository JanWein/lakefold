...
description: "Understand definitions, execution, adapters and resource ownership."
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Architecture and execution boundaries}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

For maintainers and advanced users. The [introductory lessons](get-started.html)
are sufficient for everyday work. Read this when choosing an integration or
extending the package; it explains responsibilities rather than setup steps.
...
```{r definition}
library(tidyweave)
orders <- tw_product("orders", data.frame(id = 1:2, amount = c(10, 20))) |>
...
| Enrich a primary table | `tw_add_lookup()` with explicit equality keys | Native or dm checks; same guaranteed many-to-one join |
...
both implement `tw_write_target()`.

## Choose execution defaults once

```{r execution-defaults}
execution <- tw_execution_config(quality = "native", relationships = "native")
result <- orders |> tw_run(execution = execution)
tw_collect(result)
```
...
no target remain in memory; their configured targets and layers remain unchanged. `tw_ingest()` always accepts into RAW and rejects
...
specialist checks. `tw_execution_config(quality = "pointblank", relationships = "dm")`
uses the optional engines with the same predicates and keys. Selecting an
execution default leaves the declared contract and its immutable identity
unchanged. Execution evidence separately records the resolved checking engine.
Changing the execution can still require a new explicit product code version;
changing a business promise requires a new contract version.

## Reuse expectations without inventing meaning

```{r contract-composition}
order_contract <- tw_contract("orders.schema", columns = c(id = "integer", amount = "numeric"),
  key = "id", grain = "One order")
enriched_contract <- tw_contract_update(order_contract, id = "orders.enriched.schema",
  columns = c(channel = "character"), required = c(order_contract$required, "channel"))
```

`tw_contract_update()` retains the original definition and derives an explicitly
...
Simple formulas in contracts and `tw_add_quality()` use the same normalization.

## Correct an input without reconstructing the workflow

```{r corrections}
corrected_orders <- orders |>
  tw_replace_sources(orders = data.frame(id = 1:2, amount = c(10, 25)))
corrected_orders |> tw_run() |> tw_collect()
```

A primary source supplied to `tw_product("orders", data)` defaults to the name
`orders`; `source_name` can make that name more specific. For a root product,
replacements use named source aliases; a nested product may
...
not another public workflow language. `tw_run()` is the common execution verb.
`tw_publish()` shares the approval intention across products and successful dbt
builds. It supplies a local lake target for a product when absent. Receipt
validation uses data-first `tw_ingest()` before any RAW table is written.
...
evaluates them. Unsupported verbs use `tw_add_transform(function(data) ...)`.
...
`tw_status()` retains the native `status` and adds the common `outcome` categories
`succeeded`, `blocked`, `failed` and `skipped`. `tw_lineage(result)` reads recorded
...
`tw_run(product, evidence = "runs")` adds durable local JSON evidence and a catalog
...
| Caller-supplied lake | Caller | Close with `tw_close_lake()` when finished |
...
certain targets need collection; use `tw_collect()` deliberately. The lake target
...
`tw_measure(approved, metrics = metrics, at = dates, period = "each")` computes the
metric-by-date combinations and keeps their original manifests in one measurement
set. `tw_collect()` exposes a long tibble with dimensions, `.metric`, `.period`,
...
`measurements |> tw_report_release(id, code_version = ...)` issues the original
...
information to issue that report. `tw_report_read(config, id, values_only = TRUE)`
returns the saved batch as tidy values without recalculation. The destination
can be inferred from published measurements; pass `to` when it cannot. See the
...
through `tw_capabilities()` and implement their own atomicity and cleanup. DBI
