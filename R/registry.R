registry_init <- function(lake) {
  ...
  for (name in names(schemas)) exec(lake, paste0("CREATE TABLE IF NOT EXISTS ", meta(lake, name), " (", schemas[[name]], ")"))
}
...
#' @return A tibble.
#' @export
dl_registry <- function(lake, table = c("assets", "runs", "inputs", "quality_results", "releases", "lineage_edges", "events", "reports")) {
  assert_lake(lake); table <- match.arg(table)
...
#' @return The definition, invisibly. Reusing a version with changed content errors.
#' @export
dl_register <- function(lake, object) {
  assert_lake(lake)
  if (is.null(object$id) || is.null(object$version) || is.null(object$kind)) abort("Object is not a registerable definition.")
  definition <- object; definition$config <- NULL
  h <- fingerprint(definition)
  old <- query(lake, paste("SELECT fingerprint FROM", meta(lake, "assets"), "WHERE id = ? AND version = ? AND kind = ?"), list(object$id, object$version, object$kind))
  if (nrow(old)) {
    if (any(old$fingerprint != h)) abort(paste("Definition changed without a version bump:", object$id, object$version))
  } else insert_meta(lake, "assets", list(id = object$id, version = object$version, kind = object$kind,
    owner = object$owner %||% "", description = object$description %||% "", definition = jencode(definition), fingerprint = h, registered_at = now()))
  invisible(object)
}
...
resolve_release <- function(lake, asset, release = NULL) {
  ...
  params <- list(asset)
  if (!is.null(release)) { sql <- paste(sql, "AND release_id = ?"); params <- c(params, list(release)) }
  rows <- query(lake, paste(sql, "ORDER BY published_at DESC, release_id DESC LIMIT 1"), params)
  if (!nrow(rows)) abort(paste("No published release for", asset), "dl_no_release")
  rows
}
