...
tw_contract_update <- function(
  ...
  if (!inherits(x, "tw_contract")) {
    abort("x must be a contract specification.")
  }
  ...
  if (
    ...
    abort("... must contain unique named contract arguments.")
  }
  if (
    !is.character(remove) || anyNA(remove) || !all(remove %in% names(x$columns))
  ) {
    abort("remove must name declared contract columns.")
  }
  ...
  if (
    ...
    abort("Supply required explicitly after removing a required column.")
  }
  if (
