...
#'
#' For `tw_dbt_project(lake = config)`, this function updates a deferred
...
#'   returned by [tw_ingest()] or [tw_publish()]. Names are dbt identifiers such as `orders`.
...
#' @seealso [tw_dbt_init()], [tw_dbt_contract()]
...
#' tw_dbt_sources(project, list(orders = accepted), name = "raw")
#' unlink(root, recursive = TRUE)
#' @export
tw_dbt_sources <- function(project, sources, name = "inputs") {
  if (!inherits(project, "tw_dbt_project")) {
    abort("Use tw_dbt_project() or tw_dbt_init() first.", "tw_dbt_invalid")
  }
  ident(name)
  ...
  marker <- "# Generated by tidyweave::tw_dbt_sources."
...
dbt_resolve_references <- function(references, lake) {
  lapply(references, function(reference) {
    records <- tw_releases(lake, reference$asset)
    record <- records[
...
dbt_binding_yaml <- function(resolved, name, catalog) {
  ...
  if (length(unique(schemas)) != 1L) {
    abort(
      "Each dbt source group needs one physical schema. Use tw_dbt_sources(..., name = ...) to declare separate groups for different layers.",
      "tw_dbt_invalid"
    )
  }
...
dbt_source_binding <- function(sources, name = "raw", config = NULL) {
  ...
  lake <- tw_connect_lake(config %||% references[[1L]]$config, read_only = TRUE)
