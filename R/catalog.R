...
dl_catalog_export <- function(lake, path) {
  ...
  snapshot <- stats::setNames(
    lapply(names, function(n) dl_registry(lake, n)),
    names
  )
  ...
  if (!file.rename(tmp, path)) {
    abort("Unable to publish catalog snapshot; previous file retained.")
  }
...
#' )
#' lake <- dl_connect(config)
#' app <- dl_catalog(lake, launch = FALSE)
#' class(app)
#' dl_disconnect(lake)
#' unlink(root, recursive = TRUE)
dl_catalog <- function(
  ...
  if (is.null(lake) == is.null(snapshot)) {
    abort("Supply exactly one of lake or snapshot.")
  }
