...
dl_recover <- function(
  ...
  flag(writer_stopped, "writer_stopped")
  if (!dry_run) {
    assert_writable(lake)
  }
  if (!is.character(run_ids) || anyNA(run_ids) || anyDuplicated(run_ids)) {
    abort("run_ids must be unique strings.")
  }
  invisible(lapply(staging_assets, asset_id))
  if (anyDuplicated(staging_assets)) {
    abort("staging_assets must be unique.")
  }
  ...
  plan <- function() {
    ...
    rows <- lapply(run_ids, function(id) {
      ...
      if (nrow(run) != 1L || run$status[[1]] != "running") {
        abort(paste("Recovery requires an existing running job:", id))
      }
  ...
  if (any(out$writer == "alive")) {
    abort("A selected writer is still alive. Stop it before recovery.")
  }
