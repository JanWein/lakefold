...
tw_compare <- function(
  ...
  if (is.null(from)) {
    ...
    if (position == nrow(history)) {
      abort("Comparison requires a previous release or an explicit from.")
    }
  ...
  if (is.null(key)) {
    ...
    if (nrow(definition) == 1L) {
      key <- unlist(jdecode(definition$definition[[1]])$key, use.names = FALSE)
    }
  }
  ...
  if (!all(key %in% intersect(names(types_before), names(types_after)))) {
    abort("Key columns must exist in both releases.")
  }
  for (data in list(old, new)) {
    if (
      ...
      abort("Comparison requires unique, non-missing keys in both releases.")
    }
  }
  ...
  if (
    ...
    abort("Key types differ between releases.")
  }
