...
tw_catalog_openmetadata <- function(
  ...
  scalar(database_schema, "database_schema")
  ...
  structure(
    list(
      id = paste0(
        "openmetadata-",
        substr(fingerprint(list(endpoint, database_schema)), 1L, 16L)
      ),
      endpoint = endpoint,
    ...
    class = c("tw_openmetadata_catalog", "tw_catalog_adapter")
  )
}
...
tw_publish_metadata.tw_openmetadata_catalog <- function(
  ...
  if (length(catalog$source_tables)) {
    ...
    if (is.null(output$id)) {
      abort("OpenMetadata did not return the table ID needed for lineage.")
    }
    ...
    for (name in intersect(input_names, names(catalog$source_tables))) {
      ...
      if (is.null(source$id)) {
        abort("OpenMetadata did not return the source table ID.")
      }
...
openmetadata_table <- function(catalog, metadata) {
  ...
  if (
    ...
    abort("OpenMetadata publication needs a non-empty named output schema.")
  }
  mapping <- c(
    character = "STRING",
    factor = "STRING",
