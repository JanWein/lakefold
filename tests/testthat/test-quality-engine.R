test_that("formula engines preserve lexical scope and identical row evidence", {
  ...
  evidence <- lapply(c("native", "pointblank"), function(engine) {
    run_quality(
      quality_rule("minimum", predicate, engine = engine),
      data,
      keep_agent = TRUE
    )
  })
...
test_that("simple formulas and reusable rule lists share the optional engine", {
  ...
  definition <- product("orders") |>
    add_source(data.frame(amount = c(1, 2), kind = c("a", "b"))) |>
    add_quality(checks, engine = "pointblank")
  expect_equal(
  ...
  expect_setequal(
    quality(result)$rule[quality(result)$engine == "pointblank"],
    names(checks)
  )
  reused <- quality_rule("positive", ~ amount > 0, engine = "pointblank")
  expect_identical(
    add_quality(product("x"), reused)$quality[[1]]$engine,
    "pointblank"
  )
  expect_identical(
    add_quality(product("x"), reused, engine = "native")$quality[[1]]$engine,
    "r"
  )
})
...
test_that("formula engines reject malformed outputs and keep empty evidence closed", {
  ...
  for (engine in c("native", "pointblank")) {
    for (predicate in list(~1, ~ c(TRUE, FALSE), ~ matrix(TRUE, 3, 1))) {
      error <- tryCatch(
        run_quality(quality_rule("bad", predicate, engine = engine), data),
        error = identity
      )
      expect_s3_class(error, "tw_error")
      expect_match(conditionMessage(error), "one logical value or one per row")
    }
    unknown <- run_quality(quality_rule("unknown", ~NA, engine = engine), data)
    expect_equal(unknown$n_failed, 3)
    expect_identical(unknown$status, "failed")
    scalar <- run_quality(
      quality_rule("constant", ~TRUE, engine = engine),
      data
    )
    expect_equal(scalar$n_total, 3)
    empty <- run_quality(
      quality_rule("empty", ~ amount > 0, engine = engine),
      data[0, , drop = FALSE]
    )
...
test_that("formula threshold boundaries and warning gates agree across engines", {
  ...
  for (engine in c("native", "pointblank")) {
    ...
    expect_identical(run_quality(equal, data)$status, "passed")
    warning <- quality_rule(
      "limit",
      ~ amount > 0,
      severity = "warning",
      max_failure = 0.5,
    ...
    expect_identical(run_quality(warning, data)$status, "warning")
  }
})
  ...
  for (engine in c("native", "pointblank")) {
    ...
    for (amount in list(c(1, -1), numeric())) {
      result <- product("orders") |>
        add_source(data.frame(amount = amount)) |>
        add_contract(contract(
          columns = c(amount = "numeric"),
          allow_empty = TRUE
        )) |>
        add_quality(~ amount > 0, engine = engine) |>
        set_target(structure(list(), class = "quality_gate_target")) |>
...
test_that("optional formula engines fail preflight without reading or writing", {
  ...
  definition <- product("orders") |>
    ...
    set_target(structure(list(), class = "quality_dependency_target"))
...
test_that("pointblank formulas remain lazy and ungrouped like native predicates", {
  ...
  rule <- quality_rule("minimum", predicate, engine = "pointblank")
  output <- run_quality(rule, lazy, keep_agent = TRUE)
  ...
  native <- run_quality(quality_rule("minimum", predicate), input)
