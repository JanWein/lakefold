test_that("delivery monitoring detects missing business dates without an ingest attempt", {
  ...
  expect_equal(nrow(tw_registry(f$lake, "events")), 0L)
  expect_equal(check()$status, "missing")
  expect_equal(tw_freshness(f$lake)$delivery_status, "missing")
  check()
  check()
  expect_equal(sent, 1L)
  tw_run(f$pipeline, f$lake, business_date = as.Date("2026-07-31"))
  expect_equal(check()$status, "missing")
  release <- tw_run(f$pipeline, f$lake, business_date = date)
...
test_that("failed notifications remain retryable", {
  ...
  expect_equal(tw_registry(f$lake, "events")$status, "delivery_failed")
  sent <- 0L
  tw_check_delivery(
    f$lake,
    "risk.validated",
