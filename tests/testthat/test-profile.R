test_that("profiles have bounded aggregate columns and no sample values", {
  ...
  expect_equal(profile_data(data, "amount"), profile[2, ])
  expect_equal(profile_data(dplyr::group_by(data, label)), profile)
  expect_equal(nrow(profile_data(data, character())), 0L)
  expect_error(profile_data(data, "absent"), "present in the table")
  empty <- profile_data(data[0, ])
  expect_equal(empty$n_rows, c(0, 0, 0))
  expect_equal(empty$n_distinct, c(0, 0, 0))
  expect_true(all(is.na(empty$min)))
  expect_equal(
    profile_data(data.frame(x = c(NA_real_, NA_real_)))$min,
    NA_character_
  )
})
...
test_that("database profiles equal native summaries without downloading rows", {
  ...
  expect_equal(profile_data(dplyr::group_by(table, label)), profile_data(data))
  DBI::dbExecute(con, "DELETE FROM profile_input")
  expect_equal(profile_data(table), profile_data(data[0, ]))
})
...
test_that("Arrow profiles and quality checks use the same summaries", {
  ...
  expect_equal(profile_data(table), profile_data(data))
  rule <- quality_rule("positive", ~ amount > 5)
  expect_equal(run_quality(rule, table), run_quality(rule, data))
  definition <- contract(
    columns = c(id = "integer", amount = "numeric"),
    required = character()
  )
  expect_equal(validate(table, definition), validate(data, definition))
  query <- dplyr::filter(table, amount > 5)
  expected <- dplyr::filter(data, amount > 5)
  expect_equal(profile_data(query), profile_data(expected))
  expect_equal(validate(query, definition), validate(expected, definition))
})
