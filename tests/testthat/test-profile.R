test_that("profiles have bounded aggregate columns and no sample values", {
  ...
  expect_equal(tw_profile_data(data, "amount"), profile[2, ])
  expect_equal(tw_profile_data(dplyr::group_by(data, label)), profile)
  expect_equal(nrow(tw_profile_data(data, character())), 0L)
  expect_error(tw_profile_data(data, "absent"), "present in the table")
  empty <- tw_profile_data(data[0, ])
  expect_equal(empty$n_rows, c(0, 0, 0))
  expect_equal(empty$n_distinct, c(0, 0, 0))
  expect_true(all(is.na(empty$min)))
  expect_equal(
    tw_profile_data(data.frame(x = c(NA_real_, NA_real_)))$min,
    NA_character_
  )
})
...
test_that("profiles handle factors, nested columns and exact large integers", {
  ...
  profile <- tw_profile_data(integers)
...
test_that("database profiles equal native summaries without downloading rows", {
  ...
  expect_equal(
    tw_profile_data(dplyr::group_by(table, label)),
    tw_profile_data(data)
  )
  DBI::dbExecute(con, "DELETE FROM profile_input")
  expect_equal(tw_profile_data(table), tw_profile_data(data[0, ]))
})
...
test_that("Arrow profiles and quality checks use the same summaries", {
  ...
  expect_equal(tw_profile_data(table), tw_profile_data(data))
  rule <- tw_quality_rule("positive", ~ amount > 5)
  expect_equal(tw_run_quality(rule, table), tw_run_quality(rule, data))
  definition <- tw_contract(
    columns = c(id = "integer", amount = "numeric"),
    required = character()
  )
  expect_equal(tw_validate(table, definition), tw_validate(data, definition))
  query <- dplyr::filter(table, amount > 5)
  expected <- dplyr::filter(data, amount > 5)
  expect_equal(tw_profile_data(query), tw_profile_data(expected))
  expect_equal(
    tw_validate(query, definition),
    tw_validate(expected, definition)
  )
})
