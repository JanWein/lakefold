test_that("reports escape metadata, expose the gate and never include retained agents", {
  contract <- dl_contract(
    ...
    c(id = "integer"),
    key = "id",
    rules = list(dl_rule("<script>alert(1)</script>", function(data) FALSE))
  )
  quality <- dl_validate(data.frame(id = 1L), contract)
  root <- withr::local_tempdir()
  html <- file.path(root, "quality.html")
  json <- file.path(root, "quality.json")
  dl_quality_report(quality, html)
  ...
  dl_quality_report(quality, json, format = "json")
  ...
  expect_snapshot(error = TRUE, dl_quality_report(quality, html))
})
...
test_that("native pointblank HTML can be exported from retained agents", {
  ...
  contract <- dl_contract(
    ...
    "One amount",
    c(amount = "numeric"),
    rules = list(dl_pointblank("positive", function(data) {
  ...
  dl_pointblank_report(quality, "positive", path)
