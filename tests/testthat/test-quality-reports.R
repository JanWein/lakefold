test_that("reports escape metadata, expose the gate and never include retained agents", {
  contract <- tw_contract(
    ...
    c(id = "integer"),
    key = "id",
    rules = list(tw_quality_rule("<script>alert(1)</script>", function(data) {
      FALSE
    }))
  )
  quality <- tw_validate(data.frame(id = 1L), contract)
  ...
  tw_quality_report(quality, json, format = "json")
  ...
  expect_snapshot(error = TRUE, tw_quality_report(quality, html))
})
...
test_that("native pointblank HTML can be exported from retained agents", {
  ...
  contract <- tw_contract(
    ...
    "One amount",
    c(amount = "numeric"),
    rules = list(tw_pointblank_checks("positive", function(data) {
  ...
  tw_pointblank_report(quality, "positive", path)
