test_that("good, invalid, missing and corrected deliveries preserve history", {
  ...
  first <- run(f$pipeline, f$lake, business_date = "2026-08-31")
  ...
  expect_true(nrow(registry(f$lake, "quality_results")) > 0)
  unlink(f$path)
  missing <- run(f$pipeline, f$lake, stop_on_failure = FALSE)
  expect_equal(missing$status, "missing")
  corrected <- f$good
  corrected$reserve[1] <- 150
  f$write(corrected)
  fixed <- run(f$pipeline, f$lake)
  ...
  expect_equal(
    sum(
      dplyr::collect(tbl(f$lake, "risk.validated", first$release_id))$reserve
    ),
    300
  )
  expect_equal(nrow(registry(f$lake, "releases")), 2)
  expect_equal(freshness(f$lake)$latest_attempt, "published")
})
...
test_that("schema failures and duplicate keys block publication", {
  ...
  result <- run(f$pipeline, f$lake, stop_on_failure = FALSE)
  expect_equal(result$status, "blocked")
  expect_equal(
    result$quality$status[result$quality$rule == "unique_key"],
    "failed"
  )
  expect_error(tbl(f$lake, "risk.validated"), class = "tw_no_release")
  q <- validate(data.frame(unexpected = 1), f$contract)
...
test_that("warnings and explicit count thresholds work", {
  ...
  c <- f$contract
  c$rules <- list(
    quality_rule("warning", function(x) FALSE, severity = "warning"),
    quality_rule(
      "within tolerance",
      function(x) quality_counts(1, 10),
      max_failure = .1
    )
  )
  q <- validate(f$good, c)
...
test_that("contracts cannot silently change under an existing version", {
  f <- fixture()
  on.exit(fixture_cleanup(f))
  register(f$lake, f$contract)
  changed <- f$contract
  changed$grain <- "different grain"
  expect_error(register(f$lake, changed), "version bump")
})
...
test_that("notifications deduplicate delivered events but retry transport failures", {
  ...
  run(f$pipeline, f$lake, notify = notify, stop_on_failure = FALSE)
  run(f$pipeline, f$lake, notify = notify, stop_on_failure = FALSE)
  expect_equal(sent, 1L)
  expect_true("suppressed" %in% registry(f$lake, "events")$status)
  f$write()
  ...
  expect_true("delivery_failed" %in% registry(f$lake, "events")$status)
})
...
test_that("freshness changes even when no new job runs", {
  ...
  on.exit(fixture_cleanup(f))
  run(f$pipeline, f$lake)
  expect_equal(freshness(f$lake)$freshness, "current")
  expect_equal(
    freshness(f$lake, at = Sys.time() + 72 * 3600)$freshness,
    "stale"
  )
})
...
test_that("nonexistent relative paths are frozen to the original working directory", {
  original <- getwd()
  catalog <- registry_duckdb("not-created-yet.ducklake")
  source <- source_file("test.source", "not-created-yet.csv")
