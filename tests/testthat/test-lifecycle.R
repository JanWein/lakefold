test_that("good, invalid, missing and corrected deliveries preserve history", {
  ...
  expect_equal(sum(dplyr::collect(dl_tbl(f$lake, "risk.validated"))$reserve), 300)
  bad <- f$good; bad$reserve[1] <- -100; f$write(bad)
  event <- NULL
  blocked <- dl_run(f$pipeline, f$lake, notify = function(x) event <<- x, stop_on_failure = FALSE)
  ...
  corrected <- f$good; corrected$reserve[1] <- 150; f$write(corrected)
  fixed <- dl_run(f$pipeline, f$lake)
  expect_equal(fixed$status, "published")
  expect_equal(sum(dplyr::collect(dl_tbl(f$lake, "risk.validated"))$reserve), 350)
  expect_equal(sum(dplyr::collect(dl_tbl(f$lake, "risk.validated", first$release_id))$reserve), 300)
...
test_that("retry is idempotent and never promotes an old cached release", {
  ...
  modified <- f$good; modified$reserve <- c(300, 400); f$write(modified)
  third <- dl_run(f$pipeline, f$lake)
  f$write()
  old_retry <- dl_run(f$pipeline, f$lake)
  expect_equal(old_retry$release_id, first$release_id)
  expect_equal(sum(dplyr::collect(dl_tbl(f$lake, "risk.validated"))$reserve), 700)
  expect_equal(nrow(dl_registry(f$lake, "releases")), 2)
...
test_that("schema failures and duplicate keys block publication", {
  ...
  expect_equal(result$quality$status[result$quality$rule == "unique_key"], "failed")
...
test_that("skipped, errored and empty rules never pass", {
  f <- fixture(); on.exit(cleanup(f))
  for (fn in list(function(x) NA, function(x) stop("secret or row data"), function(x) dl_quality_counts(0, 0))) {
    c <- f$contract; c$rules <- list(dl_rule("test", fn))
    q <- dl_validate(f$good, c)
    expect_true(any(q$status %in% c("error", "not_checked")))
    expect_false(dataloom:::quality_ok(q))
    expect_false(any(grepl("secret", q$message)))
  }
  empty <- dl_validate(f$good[0, ], f$contract)
  expect_false(dataloom:::quality_ok(empty))
})

test_that("warnings and explicit count thresholds work", {
  f <- fixture(); on.exit(cleanup(f))
  c <- f$contract
  c$rules <- list(dl_rule("warning", function(x) FALSE, severity = "warning"),
    dl_rule("within tolerance", function(x) dl_quality_counts(1, 10), max_failure = .1))
  q <- dl_validate(f$good, c)
  expect_true(dataloom:::quality_ok(q))
...
test_that("contracts cannot silently change under an existing version", {
  ...
  changed <- f$contract; changed$grain <- "different grain"
...
test_that("notifications deduplicate delivered events but retry transport failures", {
  f <- fixture(); on.exit(cleanup(f))
  bad <- f$good; bad$reserve[1] <- -1; f$write(bad)
  sent <- 0L
  ...
  bad$reserve[1] <- -2; f$write(bad)
  x <- dl_run(f$pipeline, f$lake, notify = function(e) stop("mail down"), stop_on_failure = FALSE)
...
test_that("freshness changes even when no new job runs", {
  ...
  expect_equal(dl_freshness(f$lake, at = Sys.time() + 72 * 3600)$freshness, "stale")
})
