test_that("minimal ingestion publishes an exact raw reference and schema", {
  ...
  accepted <- tw_ingest(orders, lake, quality = ~ amount >= 0)
  ...
  expect_equal(tw_collect(accepted), tibble::as_tibble(orders))
  ...
  expect_setequal(tw_quality(accepted)$stage, c("ingest", "candidate"))
...
test_that("native input failures never write raw and keep accepted releases", {
  ...
  expect_equal(unique(tw_quality(blocked)$stage), "ingest")
  expect_true(any(tw_quality(blocked)$status == "failed"))
  ...
  expect_equal(tw_releases(lake, "orders")$release_id, accepted$release_id)
  expect_equal(tw_collect(accepted)$amount, 10)
  expect_equal(tw_registry(lake, "runs")$status, c("published", "blocked"))
  error <- tryCatch(
    tw_ingest(
...
test_that("file readers and callbacks run once against retained original bytes", {
  ...
  accepted <- tw_ingest(path, lake, quality = check, reader = reader)
  expect_equal(accepted$asset, "orders")
...
test_that("inferred schema failures are blocked before raw without locking a failed first schema", {
  lake <- tw_open_lake(withr::local_tempdir())
  withr::defer(tw_close_lake(lake))
  accepted <- tw_ingest(data.frame(id = 1:2), lake, "orders")
  for (bad in list(
    data.frame(id = c("a", "b")),
    data.frame(id = 1L, extra = TRUE),
    data.frame(id = integer())
  )) {
    result <- tw_ingest(bad, lake, "orders", stop_on_failure = FALSE)
    expect_equal(result$status, "blocked")
    expect_equal(unique(tw_quality(result)$stage), "ingest")
    expect_false(DBI::dbExistsTable(
      lake$con,
  ...
  next_delivery <- tw_ingest(data.frame(id = "a"), lake, "new_orders")
  expect_equal(next_delivery$status, "published")
  expect_equal(tw_collect(next_delivery)$id, "a")
})
...
test_that("source functions are acquired once and default runs reevaluate captured state", {
  ...
  first <- tw_ingest(source, lake, "orders", quality = check)
  expect_equal(c(calls, checks), c(1L, 1L))
  value <- 2L
  second <- tw_ingest(source, lake, "orders", quality = check)
...
test_that("warning-only input checks accept data and are not rerun on the candidate", {
  ...
  result <- tw_ingest(data.frame(id = 1L), lake, "orders", quality = advisory)
  expect_equal(result$status, "published")
  expect_equal(calls, 1L)
  checks <- tw_quality(result)
...
test_that("config ownership and exact result collection survive later releases", {
  ...
  first <- tw_ingest(data.frame(id = 1L), config, "orders")
  expect_null(first$output_lake)
  expect_equal(first$output_config, config)
  second <- tw_ingest(data.frame(id = 2L), config, "orders")
  expect_equal(tw_collect(first)$id, 1L)
  expect_equal(tw_collect(second)$id, 2L)
...
test_that("database source factories open once and close before returning", {
  ...
  result <- tw_ingest(
    tw_source_database(factory, table = "delivery"),
    lake,
    "orders"
  )
  expect_equal(calls, 1L)
...
test_that("pointblank builds and checks once before any raw write", {
  ...
  withr::defer(tw_close_lake(lake))
  calls <- 0L
  check <- tw_pointblank_checks("amounts", function(data) {
    calls <<- calls + 1L
    pointblank::create_agent(data) |>
      pointblank::col_vals_gte(columns = "amount", value = 0)
  })
  first <- tw_ingest(data.frame(amount = 10), lake, "orders", quality = check)
  ...
  expect_equal(unique(tw_quality(blocked)$stage), "ingest")
  expect_true("pointblank" %in% tw_quality(blocked)$engine)
  expect_false(DBI::dbExistsTable(
    lake$con,
    table_id("raw", paste0("raw_", blocked$run_id))
  ))
...
test_that("cache is explicit and returns original checked contract evidence", {
  ...
  expect_error(tw_ingest(data, lake, "orders", cache = TRUE), "code_version")
  first <- tw_ingest(
    data,
    lake,
    "orders",
  ...
  expect_true(nrow(tw_quality(second)) > 0L)
  expect_equal(tw_collect(second), tibble::as_tibble(data))
})
...
test_that("reader failure retains one durable run and immutable original", {
  ...
  expect_equal(nrow(tw_registry(lake, "runs")), 1L)
...
test_that("pinned release sources retain their exact lineage in ingestion", {
  ...
  tw_ingest(data.frame(id = 2L), lake, "original")
  result <- tw_ingest(
    tw_source_release(lake, "original", original$release_id),
    lake,
    "orders"
  )
  ...
  edges <- tw_registry(lake, "lineage_edges")
  expect_true(any(
...
test_that("ingestion refuses invalid execution options and approved asset reuse", {
  ...
  withr::defer(tw_close_lake(lake))
  data <- data.frame(id = 1L)
  expect_error(tw_ingest(data, lake, business_date = 1:2), "business_date")
  expect_error(tw_ingest(data, lake, cahe = TRUE), "Unknown ingestion option")
  approved <- tw_write_data(lake, data, "orders")
  expect_error(tw_ingest(data, lake, "orders"), "distinct ingestion name")
...
test_that("product ingestion retains its input contract and quality before RAW", {
  ...
  specification <- tw_product("orders", orders, code_version = "delivery-v1") |>
    tw_add_contract(tw_contract(
      columns = c(id = "integer", amount = "numeric"),
      key = "id"
    )) |>
    tw_add_quality(~ amount >= 0)
  accepted <- specification |> tw_ingest(to = config)
  ...
  bad <- tw_product(
    ...
    tw_add_contract(specification$contract) |>
    tw_add_quality(~ amount >= 0)
  rejected <- bad |> tw_ingest(to = config, stop_on_failure = FALSE)
...
test_that("unsupported product ingestion fails before source or destination I/O", {
  ...
  plain <- tw_product("orders", source)
  transformed <- plain |> tw_add_transform(identity)
  multiple <- plain |> tw_add_source(data.frame(id = 2L))
  nested <- tw_product("nested", plain)
  targeted <- plain |> tw_set_target(config)
  cataloged <- plain |> tw_add_catalog(function(...) invisible(NULL))
  for (invalid in list(tw_product("empty"), transformed, multiple, nested)) {
    expect_error(tw_ingest(invalid, to = config), "one ordinary product source")
  }
  for (invalid in list(targeted, cataloged)) {
    expect_error(
      tw_ingest(invalid, to = config),
      "Remove product targets and catalogs"
    )
  }
  expect_error(
    tw_ingest(plain, to = config, contract = c(id = "integer")),
    "add_contract"
  )
  expect_error(tw_ingest(plain, to = config, reader = readRDS), "add_source")
  expect_error(
    tw_ingest(plain, to = config, name = "different"),
    "own ingestion name"
  )
...
test_that("data-first ingestion has a local default and checks before creating it", {
  ...
  expect_error(tw_ingest(orders, quality = ~id ~ 1), "one-sided")
  expect_false(dir.exists("tidyweave"))
  accepted <- orders |> tw_ingest()
  expect_identical(accepted$asset, "orders")
  expect_identical(accepted$backend, "duckdb")
  expect_equal(tw_collect(accepted)$id, 1L)
  elsewhere <- orders |> tw_ingest(to = "other-lake")
  expect_equal(tw_collect(elsewhere)$id, 1L)
  readonly <- tw_lake_config(path = "not-created", read_only = TRUE)
  expect_error(tw_ingest(orders, to = readonly), "writable destination")
