test_that("config release sources read approved dbt output without registry changes", {
  ...
  config <- lake$config
  close_lake(lake)
  approved <- dbt_publish(config, build, "customer_revenue")
  checksum <- digest::digest(file = config$catalog$path, algo = "sha256")
  source <- source_release(config, approved$asset, approved$release_id)
  expect_equal(inspect(source)$backend, config$backend)
  expect_false(capabilities(source)$lazy)
  ...
  result <- product("export") |> add_source(source) |> run()
  expect_equal(collect(result)$revenue, 100)
  ...
  exported <- product("database_export") |>
    add_source(source) |>
    set_target(target_database(destination, "approved")) |>
    run()
  ...
  writable <- connect_lake(config)
  withr::defer(close_lake(writable))
  expect_true(DBI::dbIsValid(writable$con))
  expect_equal(nrow(registry(writable, "runs")), 1L)
})
...
test_that("config source pins remain unchanged after later publications", {
  ...
  latest <- read_source(source_release(config, "orders"))
...
test_that("latest is resolved once and records that release on both source forms", {
  ...
  lazy <- read_source(connected)
  expect_equal(calls, 1L)
  expect_equal(attr(lazy, "tw_input_reference")$release_id, accepted$release_id)
  expect_s3_class(lazy, "tbl_sql")
  expect_true(capabilities(connected)$lazy)
  expect_true(DBI::dbIsValid(lake$con))
  expect_equal(collect(lazy)$id, 1L)
  close_lake(lake)
  calls <- 0L
  eager <- read_source(source_release(config, "orders"))
  expect_equal(calls, 1L)
  expect_equal(
...
test_that("config source provenance survives publication into a different lake", {
  ...
  withr::defer(close_lake(destination))
  result <- product("orders") |>
    add_source(source_release(config, "original", first$release_id)) |>
    set_target(destination) |>
    run()
  expect_equal(collect(result)$id, 1L)
  edges <- registry(destination, "lineage_edges")
  expect_true(any(
...
test_that("construction and validation create no storage and reject invalid inputs", {
  ...
  source <- source_release(config, "orders")
  expect_false(dir.exists(root))
  expect_invisible(check_component(source))
  expect_false(dir.exists(root))
  expect_error(read_source(source), "read-only catalog must already exist")
  expect_false(dir.exists(root))
  expect_error(
    source_release(list(), "orders"),
    "connected lake or lake_config"
  )
  invalid <- config
  invalid$backend <- "unsupported"
  expect_error(source_release(invalid, "orders"), "arg")
  expect_error(source_release(config, "orders", character()), "release_id")
  testthat::local_mocked_bindings(
...
test_that("read-only failures preserve existing files and release handles close", {
  ...
  expect_error(
    read_source(source_release(config, "orders")),
    "Registry is missing"
  )
  ...
  close_lake(lake)
  expect_error(read_source(source), "connected tw_lake")
  expect_error(
    read_source(source_release(config, "orders", "unknown")),
    class = "tw_no_release"
  )
  config$read_only <- TRUE
  expect_equal(
    read_source(source_release(config, "orders", accepted$release_id))$id,
    1L
  )
  reopened <- connect_lake(config, read_only = FALSE)
  withr::defer(close_lake(reopened))
...
test_that("config sources reuse the same publication lake without a second attachment", {
  ...
  from_open <- product("connected_copy") |>
    add_source(source) |>
    set_target(lake) |>
    run()
  expect_equal(collect(from_open)$id, 1L)
  expect_true(DBI::dbIsValid(lake$con))
  close_lake(lake)
  from_config <- product("config_copy") |>
    add_source(source) |>
    set_target(config) |>
    run()
  ...
  withr::defer(close_lake(reader))
  edges <- registry(reader, "lineage_edges")
  expect_setequal(
...
test_that("release-source subclasses keep their custom read method", {
  lake <- open_lake(withr::local_tempdir())
  withr::defer(close_lake(lake))
  source <- source_release(lake$config, "custom")
