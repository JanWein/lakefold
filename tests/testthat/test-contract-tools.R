test_that("drafts infer types only and require an explicit review transition", {
  ...
  draft <- dl_contract_from(data, "orders", "Analytics", "Orders", "One order")
  expect_s3_class(draft, "dl_contract_draft")
  expect_equal(draft$required, character())
  expect_equal(draft$key, character())
  expect_snapshot(error = TRUE, dl_validate(data, draft))
  draft$key <- "id"
  ...
  dl_expect_quality(dl_validate(data, contract))
  expect_snapshot(
    error = TRUE,
    dl_contract_from(
...
test_that("contract differences distinguish tightening from metadata and semantic changes", {
  ...
  new$owner <- "Finance"
  difference <- dl_contract_diff(old, new)
  ...
  expect_equal(nrow(dl_contract_diff(old, old)), 0L)
})
