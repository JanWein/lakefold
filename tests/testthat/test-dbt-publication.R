...
test_that("minimal dbt publication returns an exact collectable release", {
  ...
  release <- tw_dbt_publish(f$lake, f$dbt, "customer_revenue")
  ...
  expect_equal(tw_collect(release)$revenue, 100)
  expect_equal(
    tw_collect(tw_dbt_publish(f$lake, f$dbt, "customer_revenue"))$revenue,
    200
  )
})
...
test_that("config publication closes owned handles and prefers configured marts", {
  ...
  release <- tw_dbt_publish(config, f$dbt, "customer_revenue")
  expect_null(release$output_lake)
  expect_identical(release$outputs$schema, "marts")
  expect_equal(tw_collect(release)$revenue, 100)
  con <- tw_connect_lake(config)
  withr::defer(tw_disconnect_lake(con))
  expect_equal(
    tw_read_release(con, release$asset, release$release_id)$revenue,
    100
  )
})
...
test_that("automatic versions track definitions but ignore invocation timestamps", {
  f <- dbt_publication_fixture()
  withr::defer(fixture_cleanup(f))
  first <- tw_dbt_publish(f$lake, f$dbt, "customer_revenue")
  ...
  repeated <- tw_dbt_publish(f$lake, f$dbt, "customer_revenue")
  ...
  f$dbt <- dbt_publication_new_artifacts(f$dbt)
  changed <- tw_dbt_publish(f$lake, f$dbt, "customer_revenue")
  ...
  evolved <- tw_dbt_publish(f$lake, f$dbt, "customer_revenue")
  expect_false(identical(
  ...
  expect_equal(
    nrow(tw_collect(tw_dbt_publish(f$lake, f$dbt, "customer_revenue"))),
    0L
  )
})
...
test_that("invalid invocations and final contracts preserve the consumer release", {
  ...
  expect_error(
    tw_dbt_publish(f$lake, failed, "customer_revenue"),
  ...
  expect_error(
    tw_dbt_publish(f$lake, absent, "customer_revenue"),
    "result changed"
  )
  wrong <- f$dbt
  wrong$invocation_id <- "unrelated-invocation"
  expect_error(
    tw_dbt_publish(f$lake, wrong, "customer_revenue"),
    "artifacts changed"
  )
  expect_error(
    tw_dbt_publish(f$lake, f$dbt, "+customer_revenue"),
    "exact dbt unique ID"
  )
  ambiguous <- f$dbt
  ...
  expect_error(
    tw_dbt_publish(f$lake, ambiguous, "customer_revenue"),
    "result changed"
  )
  expect_error(
  ...
  checked <- tw_contract(
    ...
    rules = list(tw_quality_rule("positive", ~ revenue > 0))
  )
  blocked <- tw_dbt_publish(
    f$lake,
    f$dbt,
    "customer_revenue",
    contract = checked,
  ...
  expect_error(tw_collect(blocked), "no successful output")
  expect_equal(tw_collect(first)$revenue, 100)
  expect_identical(
