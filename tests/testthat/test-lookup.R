test_that("checked lookups preserve rows, names and groups without eager reads", {
  ...
  expect_equal(calls, 0L)
  output <- tw_run(definition) |> tw_collect()
...
test_that("partial key mappings support composite lookup grains", {
  ...
  result <- tw_product("payments") |>
    tw_add_source(input) |>
    tw_add_lookup(reference, by = c("id", month = "date")) |>
    tw_run() |>
    tw_collect()
...
test_that("orphan and missing child keys require an explicit keep policy", {
  ...
  definition <- tw_product("orders") |> tw_add_source(input)
  blocked <- definition |>
    tw_add_lookup(parent, by = "id") |>
    tw_run(stop_on_failure = FALSE)
  expect_s3_class(blocked$error$parent, "tw_lookup_unmatched")
  kept <- definition |>
    tw_add_lookup(parent, by = "id", unmatched = "keep") |>
    tw_run() |>
    tw_collect()
...
test_that("dm and native lookups enforce the same null and cardinality policy", {
  ...
  outputs <- lapply(c("native", "dm"), function(engine) {
    definition <- tw_product("payments") |> tw_add_source(input)
    output <- definition |>
      tw_add_lookup(parent, by = c("id", "month"), engine = engine) |>
      tw_run() |>
      tw_collect()
    orphan <- input
    orphan$month[[1]] <- NA_integer_
    bad <- tw_product("bad") |>
      tw_add_source(orphan) |>
      tw_add_lookup(parent, by = c("id", "month"), engine = engine) |>
      tw_run(stop_on_failure = FALSE)
    ...
    invalid <- definition |>
      tw_add_lookup(duplicate, by = c("id", "month"), engine = engine) |>
      tw_run(stop_on_failure = FALSE)
    expect_s3_class(invalid$error$parent, "tw_lookup_parent_key")
    output
  })
...
test_that("lookup references can be shared product dependencies", {
  reads <- 0L
  parent <- tw_product("customers") |>
    tw_add_source(function() {
      reads <<- reads + 1L
      data.frame(id = 1:2, label = c("a", "b"))
    })
  definition <- tw_product("orders") |>
    tw_add_source(data.frame(id = c(1L, 1L))) |>
    tw_add_lookup(parent, by = "id")
  result <- tw_run(definition)
  expect_equal(reads, 1L)
  expect_equal(tw_collect(result)$label, c("a", "a"))
})

test_that("lookup evidence belongs only to the step that checked the relationship", {
  result <- tw_product("orders") |>
    tw_add_source(data.frame(id = c(1L, 1L, 2L))) |>
    tw_add_lookup(data.frame(id = 1:2, amount = c(10, 20)), by = "id") |>
  ...
  expect_equal(tw_collect(result)$amount, c(20, 20, 40))
})

test_that("empty inputs preserve their grain and optional missing references", {
  input <- data.frame(id = integer())
  definition <- tw_product("empty") |>
    tw_add_source(input) |>
    tw_add_lookup(data.frame(id = 1L, label = "a"), by = "id") |>
    tw_add_contract(tw_contract(
      columns = c(id = "integer", label = "character"),
      allow_empty = TRUE
    ))
  result <- tw_run(definition)
  expect_equal(nrow(tw_collect(result)), 0L)
  retained <- tw_product("orders") |>
    tw_add_source(data.frame(id = 1L)) |>
    tw_add_lookup(
...
test_that("lookup engine dependencies are checked before acquiring data", {
  ...
  definition <- tw_product("orders") |>
    ...
    tw_add_lookup(data.frame(id = 1L), by = "id", engine = "dm")
...
test_that("same-database lookups remain lazy and reject implicit backend transfer", {
  ...
  step <- (tw_product("orders") |>
    tw_add_lookup(reference, by = "id"))$transforms[[
    1
  ]]
  joined <- tw_execute_transform(
    step,
    input,
    sources = list(lookup = reference)
  )
